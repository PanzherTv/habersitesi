...
// TÜRKİYE VE DÜNYADAN %100 ÇALIŞAN RSS KANALLARI (JASON FORMATI)
...
export default function Home() {
  ...
  const [doviz, setDoviz] = useState({ dolar: '34.32', euro: '37.15', altin: '3.025' });
  const [havaDurumu, setHavaDurumu] = useState([
    { sehir: "Kuşadası", derece: "28°C", durum: "Güneşli" },
    { sehir: "Aydın", derece: "31°C", durum: "Açık" },
    { sehir: "İstanbul", derece: "24°C", durum: "Bulutlu" },
    { sehir: "Ankara", derece: "22°C", durum: "Açık" }
  ]);
  ...
  useEffect(() => {
    async function tumVerileriTopla() {
      ...
      // Güvenli API üzerinden tüm haber sitelerini toplama
      for (let kaynak of HABER_KAYNAKLARI) {
        try {
          ...
          console.error(kaynak.ad + " verisi çekilemedi.");
        }
      }
  ...
  if (yukleniyor) {
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: '20px', justifyContent: 'center', alignItems: 'center', height: '100vh', fontFamily: 'sans-serif', backgroundColor: '#0a0f1d', color: '#f8fafc' }}>
        ...
        <p style={{ fontSize: '15px', fontWeight: '700', color: '#94a3b8' }}>Büyük Haber Portalı, Canlı Döviz ve Hava Durumu Derleniyor...</p>
  ...
  return (
    <main style={{ backgroundColor: '#0a0f1d', minHeight: '100vh', fontFamily: 'system-ui, -apple-system, sans-serif', color: '#f1f5f9', margin: 0, paddingBottom: '60px' }}>
      ...
      {/* 📊 CANLI DÖVİZ VE HAVA DURUMU ŞERİDİ */}
      <div style={{ backgroundColor: '#070b14', borderBottom: '1px solid #1e293b', padding: '10px 20px', fontSize: '12px', display: 'flex', flexWrap: 'wrap', justifyContent: 'space-between', gap: '15px' }}>
        <div style={{ display: 'flex', gap: '15px', alignItems: 'center' }}>
          <span style={{ color: '#94a3b8', fontWeight: 'bold' }}>📈 PİYASALAR:</span>
          ...
          <span>Gram Altın: <strong style={{ color: '#eab308' }}>{doviz.altin} TL</strong></span>
        </div>
        <div style={{ display: 'flex', gap: '15px', alignItems: 'center' }}>
          <span style={{ color: '#94a3b8', fontWeight: 'bold' }}>☀️ CANLI HAVA DURUMU:</span>
          {havaDurumu.map((h, i) => (
      ...
      <div style={{ maxWidth: '1300px', margin: '30px auto 0', padding: '0 20px' }}>
        ...
        <div style={{ display: 'flex', gap: '10px', marginBottom: '30px', borderBottom: '1px solid #1e293b', paddingBottom: '15px' }}>
          <button onClick={() => setAktifDilFiltresi('all')} style={{ padding: '8px 20px', borderRadius: '10px', border: 'none', cursor: 'pointer', fontSize: '14px', fontWeight: '700', backgroundColor: aktifDilFiltresi === 'all' ? '#e11d48' : '#131e35', color: '#fff' }}>Tüm Haberler ({filtrelenmisHaberler.length})</button>
        ...
        {mansetHaberi && !aramaMetni && (
          <section onClick={() => setSeciliHaber(mansetHaberi)} style={{ position: 'relative', width: '100%', height: '420px', borderRadius: '24px', overflow: 'hidden', marginBottom: '40px', cursor: 'pointer', boxShadow: '0 10px 30px rgba(0,0,0,0.5)' }}>
            ...
            <div style={{ position: 'absolute', bottom: '30px', left: '30px', right: '30px' }}>
              <span style={{ backgroundColor: '#e11d48', color: '#fff', fontSize: '12px', fontWeight: '800', padding: '5px 12px', borderRadius: '6px' }}>🔥 MANŞET · {mansetHaberi.sourceName}</span>
            ...
            <article key={index} onClick={() => setSeciliHaber(item)} style={{ backgroundColor: '#0d1527', borderRadius: '18px', border: '1px solid #1c2638', overflow: 'hidden', display: 'flex', flexDirection: 'column', cursor: 'pointer' }}>
              ...
              <div style={{ padding: '20px', flex: '1', display: 'flex', flexDirection: 'column', justifyBetween: 'space-between' }}>
                <div>
                  ...
                  <p style={{ fontSize: '13px', color: '#94a3b8', lineHeight: '1.5', margin: 0 }}>{item.description ? item.description.slice(0, 110) + '...' : 'Okumak için tıklayın.'}</p>
                </div>
                <div style={{ marginTop: '20px', paddingTop: '12px', borderTop: '1px solid #1c2638', display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontSize: '12px' }}>
                  {item.cevrildiMi && <span style={{ color: '#10b981', fontWeight: '700' }}>🇹🇷 Tercüme</span>}
        ...
        <div style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(5, 8, 17, 0.9)', backdropFilter: 'blur(10px)', display: 'flex', justifyContent: 'center', alignItems: 'center', zIndex: 100, padding: '20px' }} onClick={() => setSeciliHaber(null)}>
          <div style={{ backgroundColor: '#0d1527', border: '1px solid #1c2638', borderRadius: '24px', maxWidth: '640px', width: '100%', maxHeight: '82vh', overflowY: 'auto', position: 'relative' }} onClick={(e) => e.stopPropagation()}>
            ...
            <div style={{ padding: '28px' }}>
              ...
              <h2 style={{ fontSize: '20px', fontWeight: '900', margin: '18px 0 12px', color: '#fff', lineHeight: '1.4' }}>{seciliHaber.title}</h2>
